...
class VerifyContractTests(unittest.TestCase):
    ...
    def test_repository_contract_passes(self) -> None:
        ...
        self.assertGreaterEqual(report["policy_revision"], 5)
    ...
    def test_producer_and_skill_are_protected(self) -> None:
        workflow, policy, verifier = self.inputs()
        invalid = dict(policy)
        invalid["protected_paths"] = [
            item for item in policy["protected_paths"] if item not in {"producer/**", "skills/asi-verifiable-engineering/**"}
        ]
