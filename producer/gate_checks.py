#!/usr/bin/env python3
"""Deterministic stdlib-only technical checks for the BIRTH-06 producer diff."""
...
TEXT_SUFFIXES = {".py", ".yml", ".yaml", ".md", ".txt"}
LOCAL_MODULES = {
    "producer",
    "evidence_producer",
    "gate_checks",
...
class LintVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        if isinstance(node.func, ast.Name) and node.func.id in {"eval", "exec"}:
            self.errors.append(
                f"line {node.lineno}: dynamic {node.func.id} is forbidden"
            )
        self.generic_visit(node)
...
def secret_patterns() -> list[re.Pattern[str]]:
    # Prefixes are assembled so the scanner source does not self-match.
    ...
    for path in changed_paths(root, base, evaluated):
        if path.is_file() and path.suffix.lower() in TEXT_SUFFIXES:
            failures.extend(
                ...
                for error in check_text_format(path)
            )
    if failures:
        print("\n".join(failures))
        return 1
    print(
        "format_check: changed text files are UTF-8, LF, "
...
def lint_check(root: Path, base: str, evaluated: str) -> int:
    ...
    for path in files:
        failures.extend(
            f"{path.relative_to(root)}: {error}" for error in lint_python(path)
        )
    if failures:
        print("\n".join(failures))
        return 1
    ...
    for path in changed_paths(root, base, evaluated):
        ...
        for number, line in enumerate(text.splitlines(), start=1):
            if any(pattern.search(line) for pattern in patterns):
                findings.append(
                    f"{path.relative_to(root)}:{number}: high-confidence secret pattern"
                )
    if findings:
        print("\n".join(findings))
        return 1
...
def rollback_check(root: Path, base: str, head: str, evaluated: str) -> int:
    subprocess.run(
        ["git", "merge-base", "--is-ancestor", base, evaluated],
        cwd=root,
        check=True,
    )
    subprocess.run(
        ["git", "merge-base", "--is-ancestor", head, evaluated],
        cwd=root,
        check=True,
    )
    ...
    print(
        "rollback_check: reverse application of the exact base..evaluated diff "
