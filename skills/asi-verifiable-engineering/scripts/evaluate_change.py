#!/usr/bin/env python3
"""Derive an ASI approval decision from policy and primary evidence."""
...
def evaluate_change(
    ...
    if acceptance.get("line_by_line_review_default") is not False:
        blockers.append(
            "Policy must explicitly set line_by_line_review_default=false."
        )
    ...
    if acceptance.get("require_all_required_gates_passed"):
        for gate in sorted(required_gates):
            ...
            if state != "passed":
                blockers.append(
                    f"Required gate {gate} must be passed, got {state!r}."
                )
    ...
    if isinstance(commands, list):
        for command in commands:
            if isinstance(command, dict) and command.get("exit_code") != 0:
                blockers.append(
                    f"Command {command.get('name', '<unnamed>')} "
    ...
    if (
        minimum_t
        and actual_t in T_RANK
        and T_RANK[actual_t] < T_RANK[minimum_t]
    ):
        blockers.append(
            f"{risk} risk requires at least {minimum_t}, got {actual_t}."
        )

    required_independence = set(assurance.get("independence", []))
    actual_independence = set(manifest.get("independence", []))
    missing_independence = sorted(
        required_independence - actual_independence
    )
    if missing_independence:
        blockers.append(
            "Missing required independence levels: "
    ...
    if (
        required_e
        and actual_e in E_RANK
        and E_RANK[actual_e] < E_RANK[required_e]
    ):
        blockers.append(
            f"{actual_t} requires at least {required_e}, got {actual_e}."
        )
    ...
    if (
        ...
        notes.append(
            "Residual risks require explicit human risk acceptance."
        )
    ...
    if acceptance.get("require_change_budget"):
        if (
            ...
            blockers.append(
                "Change budget must be present and within_budget must be true."
            )
        elif budget.get("unexpected_files"):
            blockers.append("Change budget contains unexpected files.")

    if acceptance.get("require_test_honesty"):
        honesty = manifest.get("test_honesty")
        if not isinstance(honesty, dict) or not honesty.get("evidence"):
    ...
    if acceptance.get("require_distinct_builder_auditor"):
        if not isinstance(review, dict):
            ...
            if review.get("same_context") is not False:
                blockers.append(
                    "Builder and auditor must use independent contexts."
                )
    ...
    if risk in {"high", "critical"}:
        if human_review.get("completed") is not True:
            blockers.append(
                f"{risk} risk requires completed targeted human review."
            )
        expected_mode = "targeted_dual" if risk == "critical" else "targeted"
        if human_review.get("mode") != expected_mode:
            blockers.append(
                f"{risk} risk requires human review mode {expected_mode}."
            )
    ...
    if claimed_decision != derived_decision:
        blockers.append(
            f"Claimed decision {claimed_decision!r} does not match "
            f"derived decision {derived_decision!r}."
        )
        derived_decision = "BLOCKED"
...
def main(argv: list[str]) -> int:
    if len(argv) not in {3, 5}:
        print(
            "Usage: evaluate_change.py POLICY EVIDENCE "
            "[--expect DECISION]",
            file=sys.stderr,
        )
        return 2
    ...
    expected = (
        argv[4]
        if len(argv) == 5 and argv[3] == "--expect"
        else None
    )
    if len(argv) == 5 and argv[3] != "--expect":
        print("Only --expect DECISION is supported.", file=sys.stderr)
        return 2
