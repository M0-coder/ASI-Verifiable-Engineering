...
def _top_level_sections(text: str) -> set[str]:
    return set(re.findall(r"^([a-z][a-z0-9_]*):\s*$", text, flags=re.MULTILINE))
...
def _mapping_keys(section: str) -> set[str]:
    return set(re.findall(r"^  ([a-z][a-z0-9_]*):", section, flags=re.MULTILINE))
...
def validate_policy(path: Path) -> list[str]:
    ...
    if not risk_match:
        errors.append("repository.default_risk must be low, medium, high, or critical.")
    if not re.search(r"^  policy_owner:\s*[^\s].+$", repository, flags=re.MULTILINE):
        errors.append("repository.policy_owner is required.")
    if "- main" not in repository:
        errors.append("main must be listed under repository.protected_branches.")
    ...
    for key in REQUIRED_COMMANDS:
        match = re.search(rf"^  {key}:\s*[\"']?(.+?)[\"']?\s*$", commands, re.MULTILINE)
    ...
    for gate in REQUIRED_GATES:
        if not re.search(rf"^  {gate}:\s*(true|false)\s*$", gates, re.MULTILINE):
            errors.append(f"required_gates.{gate} must be an explicit boolean.")

    agent_permissions = _section(text, "agent_permissions")
    for permission in sorted(FALSE_AGENT_PERMISSIONS):
        if not re.search(rf"^  {permission}:\s*false\s*$", agent_permissions, re.MULTILINE):
            errors.append(f"agent_permissions.{permission} must be false.")

    exceptions = _section(text, "exceptions")
    if not re.search(r"^  allow_permanent:\s*false\s*$", exceptions, re.MULTILINE):
        errors.append("exceptions.allow_permanent must be false.")
    for key in ("require_owner", "require_expiry", "require_compensating_controls"):
        if not re.search(rf"^  {key}:\s*true\s*$", exceptions, re.MULTILINE):
            errors.append(f"exceptions.{key} must be true.")

    assurance = _section(text, "assurance")
    expected_t = {"low": "T4", "medium": "T4", "high": "T5", "critical": "T6"}
    for level, expected in expected_t.items():
        ...
        if not re.search(
            ...
            errors.append(f"assurance.{level}.minimum_t must be {expected}.")

    evidence = _section(text, "evidence")
    if not re.search(r"^  require_commit_sha:\s*true\s*$", evidence, re.MULTILINE):
        errors.append("evidence.require_commit_sha must be true.")
    if not re.search(r"^  require_exit_codes:\s*true\s*$", evidence, re.MULTILINE):
        errors.append("evidence.require_exit_codes must be true.")
    ...
    if not re.search(
        ...
        errors.append("deployment.build_once_promote_same_artifact must be true.")
    if not re.search(r"^  autonomous_production_changes:\s*false\s*$", deployment, re.MULTILINE):
